Scenario: 337 Make sure TXs are not affected after the upgrade
...
Scenario: 338 Make sure claims are not affected after the upgrade
    Given that the user it's running pocket's previous stable version, RC-0.5.2.9 or any other previus stable version.
    And user does `pocket version`
    And user receives 
    |RC-0.8.0.3|
...
Scenario: 339 Make sure claims are not affected after the upgrade, claims not present
    Given that the user it's running pocket's previous stable version, RC-0.5.2.9 or any other previus stable version.
    And user does `pocket version`
    And user receives 
    |RC-0.8.0.3|
...
Scenario: 340 Make sure unstaked APPs are removed from the state
...
Scenario: 341 Make sure that max age of evidence is enforced in blocks
    Given that the user it's running pocket's latest version, RC-0.6.0
    And user does `pocket version`
    And user receives 
    |RC-0.8.0.3|
...
Scenario: 342 Make sure security patch is functional on merkle sum index height
...
Scenario: 343 Make sure security patch is functional on blockhash for height
    Given that the user it's running pocket's older version, RC-0.6.0
    And user does `pocket version`
    And user receives 
    |RC-0.8.0.3|
...
Scenario: 344 Make sure current latest version works even for lower upgradeTX versions
    Given that the user it's running pocket's older version, RC-0.6.0
    And user does `pocket version`
    And user receives 
    |RC-0.8.0.3|
...
Scenario: 345 Make sure that max validator validation/param is working - max validators decreased.
    Given that the user it's running pocket's older version, RC-0.6.0
    And user does `pocket version`
    And user receives 
    |RC-0.8.0.3|
...
Scenario: 346 Make sure that max validator validation/param is working - max validators increased.
    Given that the user it's running pocket's older version, RC-0.6.0
    And user does `pocket version`
    And user receives 
    |RC-0.8.0.3|
...
Scenario: 347 Make sure that max validator validation/param is working - max validators decreased - node staking.
    Given that the user it's running pocket's older version, RC-0.6.0
    And user does `pocket version`
    And user receives 
    |RC-0.8.0.3|
...
Scenario: 348 Make sure that max validator validation/param is working - max validators increased - node staking.
    Given that the user it's running pocket's older version, RC-0.6.0
    And user does `pocket version`
    And user receives 
    |RC-0.8.0.3|
